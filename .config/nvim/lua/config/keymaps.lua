...
-- Firefox reload (returns focus to terminal)
vim.keymap.set("n", "<leader><F5>", function()
  ...
  if vim.v.shell_error == 0 then
    vim.notify("Firefox reloaded", vim.log.levels.INFO)
  else
    vim.notify("Failed to reload Firefox: " .. result, vim.log.levels.ERROR)
  end
end, { desc = "Reload Firefox" })
